...
class App:
    ...
    def init_gui(self):
        ...
        self.btn_toggleauto= tk.Button(self.window, text="Auto Prediction", width= 50, command=self.auto_predict_toggle, borderwidth=3, relief="raised", padx=5, pady=10)
        ...
        self.btn_class_one = tk.Button(self.window, text = self.classname_one, width= 50, command = lambda: self.save_for_class(1), borderwidth=3, relief="raised", padx=5, pady=10)
        self.btn_class_one.pack(anchor=tk.CENTER, expand= True)

        self.btn_class_two = tk.Button(self.window, text = self.classname_two, width= 50, command = lambda: self.save_for_class(2), borderwidth=3, relief="raised", padx=5, pady=10)
        self.btn_class_two.pack(anchor=tk.CENTER, expand= True)

        self.btn_train = tk.Button(self.window, text= "Train Model", width=50, command= lambda: self.model.train_model(self.counters), borderwidth=3, relief="raised", padx=5, pady=10)
        self.btn_train.pack(anchor=tk.CENTER, expand= True)

        self.btn_predict = tk.Button(self.window, text= "Predict", width=50, command=self.predict, borderwidth=3, relief="raised", padx=5, pady=10)
